...
class KeywordAnalysis(BaseModel):
    projects: List[ProjectKeywords] = Field(..., description="List of projects with their keywords")
...
class DepthAnalysis(BaseModel):
    projects: List[ProjectDepthAnalysis] = Field(..., description="List of projects with depth analysis for each keyword")
...
def to_json(data_string):  
    ...
    print("[DEBUG] to_json - Raw data_string before processing:", repr(data_string))
    
    if not data_string or not data_string.strip():
        raise ValueError("to_json: Received empty or None data_string")
    
    try:
        json_data = json.loads('[' + data_string.replace('}\n{', '}, {') + ']')
        return json.dumps(json_data, indent=4)
    except json.JSONDecodeError as e:
        print(f"[ERROR] to_json - JSON decode error: {e}")
        print(f"[ERROR] to_json - Attempted to parse: {repr('[' + data_string.replace('}\n{', '}, {') + ']')}")
        raise ValueError(f"to_json: Invalid JSON format: {e}")
...
class QuestionGenerator:
    def __init__(self, resume_content, role, company):
        ...
        self.agent1 = Agent(model=Gemini(id="gemini-2.5-flash-lite", api_key=os.getenv("GEMINI_API_KEY")), response_model=Question)
        self.agent2 = Agent(model=Gemini(id="gemini-2.5-flash-lite", api_key=os.getenv("GEMINI_API_KEY")))
        self.keyword_analyser = Agent(
            ...
            instructions = [
                ...
                "Return the output as a structured JSON with a 'projects' array containing objects with 'project_name' and 'keywords' list.",
                "Example format: {\"projects\": [{\"project_name\": \"Project Name\", \"keywords\": [\"keyword1\", \"keyword2\"]}]}",
        ...
        self.depth_analyser = Agent(
            ...
            instructions = [
                ...
                "Return structured JSON output with a 'projects' array containing objects with 'project_name' and 'analysis' (list of keywords with subtopics).",
                "Example format: {\"projects\": [{\"project_name\": \"Project Name\", \"analysis\": [{\"keyword\": \"keyword1\", \"subtopics\": [\"sub1\", \"sub2\"]}]}]}",
    ...
    def analyze_keywords(self):
        ...
        print("[DEBUG] analyze_keywords - Raw content:", repr(run.content))
        
        if not run.content:
            raise ValueError("analyze_keywords: Received empty response from keyword_analyser")
        
        # When using response_model, the content is already the validated model
        if isinstance(run.content, KeywordAnalysis):
            return run.content
        
        # Fallback: try to parse as JSON if it's a string
        if isinstance(run.content, str):
            if not run.content.strip():
                raise ValueError("analyze_keywords: Received empty string from keyword_analyser")
            try:
                data = json.loads(run.content)
                # If data is a list, wrap it in the expected structure
                if isinstance(data, list):
                    return KeywordAnalysis(projects=data)
                return KeywordAnalysis.model_validate(data)
            except json.JSONDecodeError as e:
                print(f"[ERROR] analyze_keywords - JSON decode error: {e}")
                print(f"[ERROR] analyze_keywords - Content was: {run.content}")
                raise ValueError(f"analyze_keywords: Invalid JSON response: {e}")
        
        return run.content

    def analyze_depth(self, keyword_analysis):
        """Decompose keywords into detailed sub-concepts."""
        run: RunResponse = self.depth_analyser.run(str(keyword_analysis))
        print("[DEBUG] analyze_depth - Raw content:", repr(run.content))
        
        if not run.content:
            raise ValueError("analyze_depth: Received empty response from depth_analyser")
        
        # When using response_model, the content is already the validated model
        if isinstance(run.content, DepthAnalysis):
            return run.content
        
        # Fallback: try to parse as JSON if it's a string
        if isinstance(run.content, str):
            if not run.content.strip():
                raise ValueError("analyze_depth: Received empty string from depth_analyser")
            try:
                data = json.loads(run.content)
                # If data is a list, wrap it in the expected structure
                if isinstance(data, list):
                    return DepthAnalysis(projects=data)
                return DepthAnalysis.model_validate(data)
            except json.JSONDecodeError as e:
                print(f"[ERROR] analyze_depth - JSON decode error: {e}")
                print(f"[ERROR] analyze_depth - Content was: {run.content}")
                raise ValueError(f"analyze_depth: Invalid JSON response: {e}")
    ...
    def analyze_skills(self):
        ...
        print("[DEBUG] analyze_skills - Raw content:", repr(run.content))
        
        if not run.content:
            raise ValueError("analyze_skills: Received empty response from skill_analyser")
        
        # When using response_model, the content is already the validated model
        if isinstance(run.content, SkillAnalysis):
            return run.content
        
        # Fallback: handle string responses
        if isinstance(run.content, str):
            if not run.content.strip():
                raise ValueError("analyze_skills: Received empty string from skill_analyser")
            try:
                data = json.loads(run.content)
                return SkillAnalysis.model_validate(data)
            except json.JSONDecodeError:
                # If not JSON, return the string as-is for backward compatibility
                return run.content
    ...
    def generate_interview_questions(self):
        ...
        print("[DEBUG] Starting question generation...")
        
        try:
            print("[DEBUG] Analyzing keywords...")
            self.keywords = self.analyze_keywords()
            print(f"[DEBUG] Keywords analyzed: {self.keywords}")
        except Exception as e:
            print(f"[ERROR] Failed to analyze keywords: {e}")
            raise
        
        try:
            print("[DEBUG] Analyzing depth...")
            self.depth_analysis = self.analyze_depth(self.keywords)
            print(f"[DEBUG] Depth analysis complete: {self.depth_analysis}")
        except Exception as e:
            print(f"[ERROR] Failed to analyze depth: {e}")
            raise
        
        try:
            print("[DEBUG] Analyzing skills...")
            self.skill_analysis = self.analyze_skills()
            print(f"[DEBUG] Skills analyzed: {self.skill_analysis}")
        except Exception as e:
            print(f"[ERROR] Failed to analyze skills: {e}")
            raise
        ...
        print("[DEBUG] Interview questions generated:", repr(run.content))
        
        if not run.content:
            raise ValueError("generate_interview_questions: Received empty response from generator")
        
        # Return the Questions object directly if it's already validated
        if isinstance(run.content, Questions):
            return run.content
        
        # Fallback: handle string responses
        if isinstance(run.content, str):
            if not run.content.strip():
                raise ValueError("generate_interview_questions: Received empty string from generator")
            try:
                data = json.loads(run.content)
    ...
    def generate_theoretical_interview_questions(self):
        ...
        if not run.content:
            raise ValueError("generate_theoretical_interview_questions: Received empty response from generator")
        ...
        if isinstance(run.content, str):
            if not run.content.strip():
                raise ValueError("generate_theoretical_interview_questions: Received empty string from generator")
            try:
                data = json.loads(run.content)
                return Questions.model_validate(data)
            except json.JSONDecodeError:
    ...
    def generate_skill_questions(self):
        ...
        if not run.content:
            raise ValueError("generate_skill_questions: Received empty response from generator")
        ...
        if isinstance(run.content, str):
            if not run.content.strip():
                raise ValueError("generate_skill_questions: Received empty string from generator")
            try:
                data = json.loads(run.content)
                return Questions.model_validate(data)
            except json.JSONDecodeError:
    ...
    def Generate_Situations(self):
        ...
        if not run.content:
            raise ValueError("Generate_Situations: Received empty response from generator")
        ...
        if isinstance(run.content, str):
            if not run.content.strip():
                raise ValueError("Generate_Situations: Received empty string from generator")
            try:
                data = json.loads(run.content)
                return Questions.model_validate(data)
            except json.JSONDecodeError:
