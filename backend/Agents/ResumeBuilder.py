...
def to_json(data_string):  
    # Use regular string concatenation instead of f-string for the replacement
...
class ResumeBuilder:
    ...
    def __init__(self, repos, linkedin_profile_data, role):
        ...
        self.role = role
        self.agent = Agent(model=Gemini(id="gemini-2.5-flash-lite", api_key=os.getenv("GEMINI_API_KEY")), markdown=True)
        self.projectagent = Agent(model=Gemini(id="gemini-2.5-flash-lite", api_key=os.getenv("GEMINI_API_KEY")), response_model = Projects)
        self.linkinagent = Agent(model=Gemini(id="gemini-2.5-flash-lite", api_key=os.getenv("GEMINI_API_KEY")), response_model = LinkedInProfile)
        self.skillagent = Agent(model=Gemini(id="gemini-2.5-flash-lite", api_key=os.getenv("GEMINI_API_KEY")), response_model = Skills)
